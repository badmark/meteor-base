...
for version in "${meteor_versions[@]}"; do
	...
	if ! docker build --build-arg "METEOR_VERSION=${version}" --tag badmark/meteor-base:"${version}" ./src; then
		printf "${RED}Error building Docker base image for Meteor ${version}${NC}\n"
		exit 1
	fi
done
docker tag badmark/meteor-base:"${version}" badmark/meteor-base:latest
